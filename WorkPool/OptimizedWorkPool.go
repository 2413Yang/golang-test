...
type Worker struct {
	...
	JobChannel chan Job //工作通道，每个元素是一个job，worker 私有的job通道
	exit chan bool //结束信号
}
...
func (w Worker) Start() {
	go func() {
		for {
			...
			fmt.Println("reqister private JobChannel to public WorkerPool",w)
			select{
			case job := <-w.JobChannel://收到任务
				fmt.Println("get a job from private w.JobChanne")
				fmt.Println(job)
